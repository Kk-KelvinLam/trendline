...
def main() -> None:
    ...
    if key == "shared":
        ...
        st.caption(
            "現有規則（行業）：淡區間入場，先按 High／Low 淡空間揀邊；Close q50 要同意（做多 ≥ +10bps、做空 ≤ −10bps），唔同意就觀望，唔改邊。"
            "開市已穿過入場價唔追。"
            "近期綜合 MAE（0.4 High＋0.4 Low＋0.2 Close；20 日、至少 10 個樣本）高於 2.5% 則整張觀望；"
            "高於 1.8% 則唔標高信心。"
        ...
        st.caption(
            "現有規則（個股）：淡區間入場，先按 High／Low 淡空間揀邊；Close q50 要同意（做多 ≥ +10bps、做空 ≤ −10bps），唔同意就觀望，唔改邊。"
...
def _render_ledger() -> None:
    ...
    st.caption(
        ...
        "三套系統賽馬：共用無 Close 閘；行業／個股 Close 只可否決、唔可改邊。"
