...
2. **行業模型** — sector-family cards（High/Low 空間優先；Close ±10bps 只可否決）  
3. **個股模型** — per-stock（High/Low 空間優先；Close ±10bps 只可否決；薄歷史 fallback shared）  
